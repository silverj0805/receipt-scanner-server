...
export function buildSummary(thisMonth: SummarySource[], lastMonth: SummarySource[]) {
  const total = thisMonth.reduce((sum, r) => sum + r.amount, 0);
  const lastTotal = lastMonth.reduce((sum, r) => sum + r.amount, 0);
  const deltaPercent = lastTotal === 0 ? 0 : Math.round(((total - lastTotal) / lastTotal) * 100);
  ...
  return { total, deltaPercent, byCategory };
}
